...
import { apiFetch, gzipJsonRequest } from './http'
...
export async function createDiagram(title: string, code: string): Promise<DiagramDto> {
  ...
  const res = await apiFetch('/api/diagrams', await gzipJsonRequest('POST', { title, code }))
...
export async function updateDiagram(id: number, title: string, code: string): Promise<DiagramDto> {
  const res = await apiFetch(`/api/diagrams/${id}`, await gzipJsonRequest('PUT', { title, code }))
