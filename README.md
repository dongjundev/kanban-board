...
- **메모 · 파일**: 탭에서 감춰진 페이지로, **`/memo` 주소로 접근**합니다(들어가 있는 동안만 '메모' 탭이 표시됨). 텍스트 메모 작성/삭제, 파일 업로드/다운로드/삭제. 서버 저장(메모·메타데이터는 PostgreSQL, 파일 바이트는 백엔드 볼륨) — 백엔드 실행 시에만 동작
- **다이어그램**: 상단 '다이어그램' 탭에서 mermaid 코드를 입력하면 300ms 디바운스로 미리보기가 갱신됨(문법 오류는 메시지로 표시하고 직전 그림 유지, 겹친 엣지 라벨은 자동으로 세로 분리). 코드 맨 위 frontmatter에 `layout: elk`를 지정하면 **ELK 레이아웃 엔진**으로 배치 — 간선이 많아 노드가 멀리 흩어지는 큰 차트가 훨씬 조밀해짐(flowchart·상태 다이어그램 지원, 나머지 타입은 기본 dagre로 자동 폴백. 무거운 elkjs 번들은 ELK 차트를 처음 그릴 때만 지연 로드). 미리보기는 **휠로 확대/축소(커서 기준, 5%~3000%), 드래그로 이동**. SVG를 실제 크기 기준으로 렌더해 배율 표시가 정직하며, 새로 열면 **화면 맞춤**(전체 구조)으로 시작하고 버튼으로 **화면 맞춤 ↔ 실제 크기(100%, 글자 원래 크기) ↔ 전체 화면**(Esc로 종료)을 오갈 수 있음. 코드·미리보기 사이 **분할선을 드래그해 좌우 너비 조절**(더블클릭 5:5 복귀, ←/→ 키 지원, 비율은 `kanban-mermaid-split`에 기억). 제목을 붙여 서버(PostgreSQL `diagram` 테이블)에 저장하고 목록에서 불러오기·수정·삭제. 테마를 토글하면 미리보기도 즉시 다시 그려집니다. 편집 중 초안은 localStorage(`kanban-mermaid-draft`)에도 보관되며, **백엔드가 없으면 저장 UI만 감춰지고 편집·미리보기는 그대로 동작**
- **로그인**: 서버에 계정을 설정하면(`APP_AUTH_PASSWORD`) 로그인 화면이 뜨고 `/api/**` 전체가 세션 인증으로 보호됩니다. 로그인 전에는 보드·메모·차트 데이터에 접근할 수 없고, 앱도 마운트되지 않습니다. 환경변수를 설정하지 않으면 인증이 꺼진 상태로 동작(로컬 개발·E2E)
